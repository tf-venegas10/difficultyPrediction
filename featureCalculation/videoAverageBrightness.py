...
cur.execute("SELECT * FROM VIDEO_QUALIFICATION VQ WHERE QUALIFICATION_AMOUNT>0 AND VQ.VIDEO_ID NOT IN "
            "(SELECT VIDEO_ID FROM FEATURES_PER_VIDEO WHERE FEATURE_ID=38);")
for row in cur.fetchall():
    if str(row[0]) in ids:
...
img_path = (ospath + "/InitialData/image5.jpg")  # .replace("/", "\\")
processed = 0

output = open("Video_Average_Brightness.sql", "a")
output.write(
        ...
        if file.endswith(".mp4"):
            ...
            for key in vids.keys():
                ...
                if name.startswith(key):
                        ...
                        if ret:
                            if counti % interval == 0:
                                try:
                                    ...
                                    average += calculate_brightness(img)
                                    amount += 1
                                except OSError:
                    ...
                    output.write(
                        "INSERT INTO FEATURES_PER_VIDEO (feature_id, video_id, value) VALUES (38, " + str(
