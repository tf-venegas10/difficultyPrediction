...
cur.execute("SELECT * FROM VIDEO_QUALIFICATION VQ WHERE QUALIFICATION_AMOUNT>0 AND VQ.VIDEO_ID NOT IN "
            "(SELECT VIDEO_ID FROM FEATURES_PER_VIDEO WHERE FEATURE_ID=40);")
for row in cur.fetchall():
    if str(row[0]) in ids:
...
img_path = (ospath + "/InitialData/image3.jpg")  # .replace("/", "\\")
processed = 0
            ...
            for key in vids.keys():
                ...
                if name.startswith(key):
                        ...
                        if ret:
                            if counti % interval == 0:
                                try:
                                    ...
                                    average += calculateAverageHue(hsv)
                                    amount += 1
                                except OSError:
                    ...
                    if not amount == 0:
                        ...
                        average = 0.0
                    print("AVERAGE HUE: " + str(average))
                    processed = processed + 1
                    print("Processed: " + str(processed) + "/" + str(vids_amount))
                    cap.release()
                    cv2.destroyAllWindows()
                    output.write(
                        "INSERT INTO FEATURES_PER_VIDEO (feature_id, video_id, value) VALUES (40, " + str(
